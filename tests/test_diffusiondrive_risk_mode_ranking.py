...
class RiskModeRankingTest(unittest.TestCase):
    ...
    def test_uncertain_candidate_does_not_override_unsafe_base(self):
        selection = select_risk_ranked_mode(
            torch.tensor([[3.0, 2.9]]),
            torch.tensor([[1.0, -0.2]]),
            topk=2,
            config=SimpleNamespace(
                risk_rank_use_inference_path_guard=False,
                risk_rank_candidate_safe_probability_threshold=0.35,
            ),
        )
        self.assertEqual(selection["selected_mode"].item(), 0)
        self.assertEqual(selection["qualified_candidate_count"].item(), 0)
