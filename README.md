# 新 3.1.05_3：3.22 生成器刹车时机监督＋固定 PCS/TRV

本版纠正上一版实验方向：将原 3.22 的训练损失接到轨迹生成器，
保留已达到 89.181861 PDM 的 PCS＋TRV 权重、三风险头及否决阈值。
不增加筛选器特征、不训练风险头、不加入新的推理后处理。
...
以下为继承的原 PCS/TRV 实验记录，89.181861 不是本版的新实验结果。

## 原 3.1.05_2：三因子风险否决器（TRV）
