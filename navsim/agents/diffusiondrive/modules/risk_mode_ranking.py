...
from navsim.agents.diffusiondrive.modules.risk_mask_observer import (
    RISK_MASK_OBSERVER as _RISK_MASK_OBSERVER,
)
...
def select_risk_ranked_mode(
    ...
    threshold = float(_cfg(config, "risk_rank_unsafe_probability_threshold", 0.5))
    predicted_safe = (candidate_unsafe_probability < threshold) & eligible
    base_mode = base_logits.argmax(dim=-1)
...
def compute_risk_mode_ranking_loss(
    ...
    _RISK_MASK_OBSERVER.update(
        targets=targets,
        scene_active=mined["scene_active"],
        pair_mask=safety_pair_mask,
