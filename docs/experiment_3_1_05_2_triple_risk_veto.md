...
最佳 checkpoint 为 epoch 2，val 校准得到 NC、DAC、TTC 阈值均为 0.5。完整
navtest 共 12,146 个场景：

| 输出 | PDM | 相对 base | 相对 PCS |
| --- | ---: | ---: | ---: |
| base selector | 88.488936 | — | -0.586635 |
| 原 PCS | 89.075571 | +0.586635 | — |
| TRV | **89.181861** | **+0.692925** | **+0.106290** |

TRV 在 11,857 次 PCS 换选中否决 448 次，保留 11,409 次。新增零分从 231 降至
199，PCS 原本救回的零分从 193 降至 177。两者合并后，相对 PCS 净减少 16 个零分，
说明风险否决获得正收益，同时付出了 16 个有效救援被回退的代价。逐场景收益来源仍需
结合 `paired_results.csv` 分解，不能仅由汇总结果断言三个风险头各自的贡献。
...
作废该实验，不继续在 navtest 上反复调阈值。本次结果同时满足两项标准，实验保留。
