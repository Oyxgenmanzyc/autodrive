...
Put both files under `$NAVSIM_EXP_ROOT/pretrained` as `resnet34.a1_in1k.bin` and `kmeans_navsim_traj_20.npy`. You can instead set `DIFFUSIONDRIVE_BKB_PATH` and `DIFFUSIONDRIVE_PLAN_ANCHOR_PATH` to explicit file paths. The code now fails with a direct setup error when a required local asset is unavailable; no source edit is required.
